...
vim.api.nvim_create_user_command('StatusReport', function()
	...
	local report = {}
	local report_sections = { "Active", "Reviewing", "Blocked", "Completed", "Approved", "Next", }
	for _, section in ipairs(report_sections) do
		report[string.upper(section)] = {}
	end
	...
	for _, heading in ipairs(report_sections) do
		vim.api.nvim_buf_set_lines(buf, -1, -1, true, { "* " .. heading })

		-- Emit the action
		for _, todo in ipairs(report[string.upper(heading)]) do
